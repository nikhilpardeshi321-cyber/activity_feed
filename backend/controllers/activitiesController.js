...
const createActivity = async (req, res) => {
  try {
    const { tenantId, actorId, actorName, type, entityId, metadata } = req.body;
...
const getActivities = async (req, res) => {
  try {
    const { cursor, limit = 20, tenantId } = req.query;

    // Tenant isolation is mandatory
    if (!tenantId) {
      return res.status(400).json({
        status: false,
        message: 'tenantId query parameter is required for tenant isolation'
      });
    }
